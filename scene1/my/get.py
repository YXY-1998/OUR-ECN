...
def handle_pkt(pkt):
    ...
    # If one second has passed, the number of packets captured per second is calculated and printed, and the counter and start time are reset
    if elapsed_time >= 1:
        th = pkt_count/elapsed_time
        pkt_count = 0
        with open('data/throughput-my.txt', 'a') as f:
            f.write(str(th)+'\n')
        start_time = time.time()
    ...
    t=t*1000#ms
    print(t)
    with open('data/rtt-my.txt', 'a') as f:
        f.write(str(t)+'\n')
    ...
    if pkt.haslayer(IPOption_INT):
        ...
        with open('data/qdepth1-my.txt', 'a') as f:
            f.write(str(qdepth1)+'\n')
        with open('data/qdepth2-my.txt', 'a') as f:
            f.write(str(qdepth2)+'\n')
